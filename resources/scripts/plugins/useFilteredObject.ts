...
export default <T extends {}>(data: T): T => {
	...
	const removeEmptyValues = (input: any): any =>
		Object.entries(input)
			.filter(([_, value]) => !empty.includes(value))
			.reduce((obj, [k, v]) => {
				const parsed = isObject(v) ? removeEmptyValues(v) : v;

				return isObject(parsed) && isEmptyObject(parsed)
					? obj
					: { ...obj, [k]: parsed };
			}, {} as any);

	return removeEmptyValues(data) as T;
};
